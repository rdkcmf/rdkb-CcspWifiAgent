...
if [ "$sta1" != "" ] ; then
	...
	echo "WIFI_MAC_1:$mac1"
	rssi1=`echo "$sta1" | cut -d' ' -f4 | tr '\n' ','`
	echo "WIFI_RSSI_1:$rssi1"
	rxrate1=`echo "$sta1" | cut -d' ' -f3 | tr '\n' ','`
	echo "WIFI_RXCLIENTS_1:$rxrate1"
	txrate1=`echo "$sta1" | cut -d' ' -f2 | tr '\n' ','`
	echo "WIFI_TXCLIENTS_1:$txrate1"
	...
	echo "WIFI_RXTXCLIENTDELTA_1:$rxtxd1"
fi

if [ "$sta2" != "" ] ; then
        mac2=`echo "$sta2" | cut -d' ' -f1 | tr '\n' ','`
        echo "WIFI_MAC_2:$mac2"
        rssi2=`echo "$sta2" | cut -d' ' -f4 | tr '\n' ','`
        echo "WIFI_RSSI_2:$rssi2"
        rxrate2=`echo "$sta2" | cut -d' ' -f3 | tr '\n' ','`
        echo "WIFI_RXCLIENTS_2:$rxrate2"
        txrate2=`echo "$sta2" | cut -d' ' -f2 | tr '\n' ','`
        echo "WIFI_TXCLIENTS_2:$txrate2"
	...
	rm /tmp/rxx2 /tmp/txx2
        echo "WIFI_RXTXCLIENTDELTA_2:$rxtxd2"
fi
